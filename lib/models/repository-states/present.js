...
export default class Present extends State {
  ...
  async _getFilePatchForPath(filePath, options) {
    if (options.amending) {
  ...
  async getFilePatchForPath(filePath, {staged, amending, content, context} = {staged: false, amending: false}) {
    if (typeof content === 'string') {
      // Don't use cache if getting diff from unsaved changes
      return await this._getFilePatchForPath(filePath, {staged, amending, content, context});
    }
    return this.cache.getOrSet(Keys.filePatch.oneWith(filePath, {staged, amending, context}), async () => {
      return await this._getFilePatchForPath(filePath, {staged, amending, content, context});
    });
  }
...
const Keys = {
  ...
  filePatch: {
    _optKey: ({staged, amending, context}) => {
      const suffix = typeof context === 'number' ? `${context}` : '';
      if (staged && amending) {
        return 'a' + suffix;
      } else if (staged) {
        return 's' + suffix;

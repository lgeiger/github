...
export default class GitShellOutStrategy {
  ...
  async getDiffForFilePath(filePath, {staged, baseCommit, content, context} = {}) {
    let args = ['diff', '--no-prefix', '--no-ext-diff', '--no-renames', '--diff-filter=u'];
    if (typeof context === 'number') { args.push(`--unified=${context}`); }


    const normalizedPath = toGitPathSep(filePath);
    const [headBlob, currentBlob] =
      !staged && typeof content === 'string'
        ? (await Promise.all([this.getBlob(normalizedPath, baseCommit), this.createBlob({stdin: content})])) : [];

    if (headBlob && currentBlob) {
      args.push(headBlob);
      args.push(currentBlob);
    } else {
      if (staged) { args.push('--staged'); }
      if (baseCommit) { args.push(baseCommit); }
      args = args.concat(['--', normalizedPath]);
    }
  ...
  async getBlob(filePath, baseCommit = 'HEAD') {
    let output;
    try {
      output = (await this.exec(['rev-parse', `${baseCommit}:${toGitPathSep(filePath)}`])).trim();
    } catch (e) {
      if (e.stdErr && e.stdErr.match(/fatal: Path * exists on disk, but not in 'HEAD'./)) {
        output = null;
      } else {
        throw e;
      }
